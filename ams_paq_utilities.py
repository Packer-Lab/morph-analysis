...
def date(datapath):
	date = datapath.split('/')[1].split('_')[0]
...
# def make_paq_df_single(datapath):

# 	df = pd.DataFrame()

# 	for path in datapath:
# 		if 'allparams' in path:
# 			data = p2p(path,plot=True)
# 			df['Voltage'] = get_voltage(data)
# 			df['Current'] = get_current(data)
# 			df['time'] = df.index/data['rate']
# 			df['mouseID'] = path.split('_')[3]
...
def make_paq_df_single(datapath):
	...
	assert 'allparams' in datapath
	data = p2p(datapath,plot=True)
	df['Voltage'] = get_voltage(data)
	df['Current'] = get_current(data)
	df['time'] = df.index/data['rate']
...
def Baselineshift(df):
	Baselineshift = np.mean(df['Voltage'][0:5000]) - np.mean(df['Voltage'][5001:])
	return Baselineshift

def Rheobase(df):
	rheobase = np.mean(df['Current'][282000:284000])

	return rheobase 

def find_spikes_range(df):
	AP_range = df['Voltage'][567020:572020]
...
def AP_half_Width(df):
	...
	AP_hw = (hw_time_2 - hw_time_1)*1000
...
def Refractory_period(df):
	...
	return fAHP_min,fAHP,sAHP_min,sAHP
...
def ISIs(df):
	...
	return max_ISI,min_ISI,mean_ISI

def max_FR(df):
	maxFR = 1/(ISIs(df)[1]/1000)

	return maxFR

def min_FR(df):
	minFR = 1/(ISIs(df)[0]/1000)

	return minFR

def avg_FR(df):
	meanFR = 1/(ISIs(df)[2]/1000)

	return meanFR

def FR_accomodation(df):
	R4 = rheobasex4_df(df)
	peaks = find_peaks(R4['Voltage'], height = 0.0,distance=50)
	ISIs = []
	peaks100 = []
	...
	FRaccom = 100*((len(peaks100) - len(peaks400))/len(peaks100))
	return FRaccom

def make_param_df(df):

	Parameter_dict ={
		'Recording_Date':get_date(df),
                 ...
                 'Min_FR': min_FR(df),
                 # 'Mean_FR': avg_FR(df),
                 'FR_Accomodation': FR_accomodation(df),
                 'Input_Resistance':input_resistance(df),
}

	df_params = pd.DataFrame.from_dict(Parameter_dict,orient='index',columns = df.mouseID.unique())
